...
/*
========================================================
Tipo de Atualização - 13 (Inclusão/Alteração - Fase 12)
========================================================
*/
...
if @pk_trabalho_fase_manut12 is null begin
	if @fk_trabalho is null begin
		select '6' Status,'Se não informar o parâmetro < @pk_trabalho_fase_manut12 >, informe obrigatoriamente o parâmetro < @fk_trabalho >!' mensagem
		return
	end 
	if @fk_fase is null begin
		select '6' Status,'Se não informar o parâmetro < @pk_trabalho_fase_manut12 >, informe obrigatoriamente o parâmetro < @fk_fase >!' mensagem
		return
	end
end
else begin
	if not exists(select 1 from VIW_GRF_Trabalhos_Fases_Manut12 where pk_trabalho_fase_manut12 = @pk_trabalho_fase_manut12) begin
		select '6' Status, 'O registro < @pk_trabalho_fase_manut12 > = ' + convert(varchar,@pk_trabalho_fase_manut12) + ', não foi localizado na view < VIW_GRF_Trabalhos_Fases_Manut12 >!' mensagem
		return
	end
	if @fk_trabalho is not null begin
		select '6' Status,'Se informar o parâmetro < @pk_trabalho_fase_manut12 >, não informar o parâmetro < @fk_trabalho >!' mensagem
		return
	end 
	if @fk_fase is not null begin
		select '6' Status,'Se informar o parâmetro < @pk_trabalho_fase_manut12 >, não informar o parâmetro < @fk_fase >!' mensagem
		return
	end 
end
...
BEGIN TRY 
	BEGIN TRANSACTION
		if @pk_trabalho_fase_manut12 is null begin
			...
			-- Insere a Fase de Manutenção 12 Antes da Alteração (Histórico)
			...
			select @fk_trabalho_fase_manut12_alt_seq  -- Guarda a Referência do Registro Sequencialmente
				  ,fk_trabalho
				  ,fk_fase
				  ,fk_operador
				  ,fk_avaliacao_tipo
			...
			-- Altera a Fase de Manutenção 12
